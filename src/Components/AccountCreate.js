...
export default function AccountCreate() {
  ...
  const HandleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setusernameAlert("border border-red-200");
        setusernameAlreadyExistsMessage("Username already exists!..");
      }
    } catch (error) {
      setLoading(false);
      // console.error("Error:", error);
      setusernameAlert("border-b shadow-lg border-red-400");
      setusernameAlreadyExistsMessage("username already exists!..");
    }
  };
  ...
  const handleChange = (event) => {
    ...
    if (name === "username") {
      ...
      setpasswordBorder("border-b shadow shadow-red-300 border-red-400");
    } else if (name === "password" && value.length === 4) {
      setpasswordBorder("border-b  shadow shadow-orange-400 border-orange-300");
    } else if (name === "password" && value.length === 8) {
      setpasswordBorder("border-b shadow shadow-green-300 border-green-300");
    } else {
      setFormData3((prevData) => ({
        ...prevData,
        [name]: value,
  ...
  return (
    <>
      <NavBar />
      <div className=" font-Sriracha sm:flex sm:w-9/12 md:w-3/5 lg:w-2/5 mx-auto pt-20 sm:pt-40 md:pt-40 lg:pt-[180px]">
          ...
          <form action="#" onSubmit={HandleSubmit} className=" relative">
            <div className="mb-2 flex mx-auto w-10/12 sm:w-10/12  hover:border-[#4649486c] border-b border-[#27574d82]  overflow-hidden">
              <div className="w-2/12 sm:w-1/12 lg:w-2/12 bg-[#ffffffad] border-[#00ffca] flex justify-center items-center">
                <img src={signatureImg} alt="userImg" className=" size-5   " />
              </div>
              <input
                className="px-3 w-10/12 pt-1 pb-0 text-xs sm:text-sm sm:w-11/12 lg:w-10/12 outline-none  bg-[#8ef0dc05] hover:bg-white duration-500"
                type="text"
                name="firstname"
                id="firstname"
            ...
            >
              ...
              <input
                className="  px-3 w-9/12 pt-1 pb-0 text-xs sm:text-sm sm:w-10/12 lg:w-9/12 outline-none bg-[#8ef0dc05] hover:bg-white duration-500"
              ...
              <div className="w-1/12 sm:w-1/12 lg:w-1/12 bg-[#8ef0dc05] cursor-pointer flex r lg:justify-left items-center">
                <IoIosEye
                  className=" text-gray-500 size-4 sm:size-5 pr-1 "
                  onClick={() => {
            ...
            <div className="flex mb-2 mx-auto w-10/12 sm:w-10/12  overflow-hidden">
              <div className="px-3 cursor-pointer text-sm w-full pt-1 pb-1 outline-none text-center  bg-[#00ffcc49] hover:bg-[#00ffca] duration-500">
